...
// Small-talk, follow-ups, and provider fallback.
// ==========================================================
...
async function handler(req, res) {
  ...
  try {
    ...
    if (question === "__ping__") {
      ...
      res.end(JSON.stringify({ ok: true, route: "/api/ai-expert", method: req.method }));
      return;
    }
    ...
    if (st) {
      ...
      res.end(JSON.stringify({
        answer: st.reply, provider: "smalltalk", topic: "general", sources: [], followups: buildFollowups("all"),
      }));
      return;
    }
...
/* -------- attach runtime to the handler (CJS pattern) ------- */
