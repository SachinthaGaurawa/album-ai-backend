// ==========================================================
// /api/ai-expert.cjs — Text Q&A API (CommonJS for Vercel)
// RAG over internal KB + uploaded PDFs (via /api/docs.json)
// Friendly small-talk, follow-ups, personalization, fallbacks
// ==========================================================
...
// Robust body parsing (req.body can be empty on Vercel)
...
const OWNER = {
  ...
  tagline: "Engineer and creator focused on autonomous driving, embedded AI, and safety systems."
};
...
const KB = {
  ...
  docs: [
    { id:"aavss-overview", topic:"aavss", title:"AAVSS — Overview",
      text: "AAVSS is a real-time safety and perception stack. Core: multi-sensor fusion (LiDAR + mmWave radar + RGB camera). Embedded target: NVIDIA Jetson (Nano-class) with TensorRT. Typical latency sub-100 ms at 10–20 FPS depending on model sizes." },
    { id:"aavss-sensors", topic:"aavss", title:"AAVSS — Sensors & Roles",
      text: "Roles:\n• LiDAR → 3D structure, free-space.\n• mmWave radar → range+velocity, robust in rain/fog.\n• RGB camera → lights/signs, lanes, VRU detection.\nExact SKUs vary by build; add your sensor models to docs.json/PDF for precise answers." },
    { id:"aavss-pipeline", topic:"aavss", title:"AAVSS — Fusion Pipeline (high level)",
      text: "Calibration → time-sync → per-sensor detection/feature extraction → association → tracking → risk scoring/alerts (HUD/in-cabin)." },
    { id:"sld-overview", topic:"sldataset", title:"Sri Lanka Autonomous Driving Dataset — Overview",
      text: "Open driving dataset across Sri Lankan road scenarios (urban/rural; rain, fog, night). Includes annotations for lanes, signs, hazards. Add PDFs for exact specs." },
    { id:"about-owner", topic:"about", title:"Ownership & Concept",
      text: `Owner/Concept/Manufacturer: ${OWNER.fullName}. Roles: ${OWNER.roles.join(", ")}. Projects: ${OWNER.projects.join(", ")}.` },
    { id:"about-whois", topic:"about", title:`Who is ${OWNER.fullName}?`,
      text: `${OWNER.fullName} — ${OWNER.tagline} Leads development and product direction for AAVSS and the Sri Lanka dataset.` },
  ],
};

/* ----------- pull uploaded PDF chunks from your API ------- */
async function readDocsStore() {
  try {
    ...
    const j = await r.json(); return (j && Array.isArray(j.docs)) ? { docs: j.docs } : { docs: [] };
  } catch { return { docs: [] }; }
}

/* -------------- retrieval scoring & context build --------- */
function scoreText(qTokens, text) {
  const t = normalize(text); let s=0;
...
async function topK(question, k=8, topic="all") {
  ...
  const kbRanked = kbPool.map(d => ({
    type:"kb", id:d.id, title:d.title, text:d.text, s: scoreText(qTok, `${d.title}\n${d.text}`)
  })).filter(x=>x.s>0);

  const store = await readDocsStore();
  const pdfRanked = (store.docs || []).map(ch => ({
    type:"pdf", id:ch.id, title:ch.title, text:ch.text, page:ch.page, url:ch.url,
...
async function buildContext(question, topic){
  ...
  const maxS = Math.max(...ranked.map(r=>r.s), 1);
  const confidence = Math.min(1, ranked.length ? ranked.reduce((a,r)=>a + r.s/maxS, 0) / ranked.length : 0);
...
async function askGroq({ system, user, signal }) {
  ...
  if (!r.ok) throw new Error(`Groq HTTP ${r.status}`); const j = await r.json();
...
async function askDeepInfra({ system, user, signal }) {
  ...
  if (!r.ok) throw new Error(`DeepInfra HTTP ${r.status}`); const j = await r.json();
  return (j?.choices?.[0]?.message?.content ?? "").trim();
}
...
async function askGemini({ system, user, signal }) {
  ...
  if (!r.ok) throw new Error(`Gemini HTTP ${r.status}`); const j = await r.json();
...
function systemPrompt(topic){
  return [
    ...
    "Respond ONLY with facts found in the Knowledge Base (KB) below.",
    "If something isn't in the KB, say so briefly and invite the user to upload/ingest a PDF.",
    "Prefer short paragraphs and bullets; **bold** key terms; friendly emojis sparingly.",
...
function userPrompt(question, ctx){
  return ['KB:','"""', ctx || 'NO CONTEXT','"""','',
    `User question: ${question}`,'',
    'Instructions:',
    '- Use only KB facts; do not invent specifics.',
    '- If unknown, say so and suggest adding the detail.',
...
function buildFollowups(topic){
  if (topic==='aavss') return [
    ...
    "Shall I outline calibration steps?",
    "Do you want safety alert thresholds and examples?",
    "Upload your exact sensor SKUs for model-specific notes?"
  ];
  if (topic==='sldataset') return [
    "Do you need train/val/test splits?",
    "Should I list annotation formats and examples?",
    "Want evaluation metrics to track?",
    "Need a license summary?"
  ];
  return [
    "Want an overview of AAVSS vs the Dataset?",
    "Shall I suggest next steps or a quick start?",
    "Do you want me to generate or browse images?"
  ];
}
...
function smallTalk(question){
  ...
  if (/^(hi|hey|hello|ayubowan|good (morning|evening|afternoon))\b/.test(q))
    return { type:'greet',
      reply: `Hi there! I'm your assistant 🤝  Ask me about **AAVSS**, the **Sri Lanka dataset**, or upload a PDF for deeper answers.\nI can also **generate images** or **browse references**. What shall we do first?` };
  ...
  try {
    ...
    for (const p of providers) {
      try {
        ...
        t.clear(); if (answer) { provider = p.name; break; }
      } catch { /* try next */ }
    }
    ...
    if (!answer) {
      const stitched = ids.map((id,i) => {
        ...
        if (kind === "pdf") { const title = rest.split("|")[0]; return `(${i+1}) ${title} (from PDF)`; }
        return "";
      }).filter(Boolean).join("\n\n");
    ...
    res.writeHead(500, headers); res.end(JSON.stringify({ error: msg }));
  }
};
