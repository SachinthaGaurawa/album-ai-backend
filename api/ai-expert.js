...
const OWNER = {
  ...
  projects: ["AAVSS (Advanced Autonomous Vehicle Safety System)","Sri Lanka Autonomous Driving Dataset"],
...
const KB = {
  ...
  docs: [
    { id:"aavss-overview",topic:"aavss",title:"AAVSS — Overview",text:"AAVSS (Advanced Autonomous Vehicle Safety System) is a real-time safety and perception stack. Core: multi-sensor fusion (LiDAR + mmWave radar + RGB camera). Embedded target: NVIDIA Jetson (Nano-class) with TensorRT optimizations. Typical end-to-end latency sub-100 ms at 10–20 FPS depending on model sizes." },
    { id:"aavss-sensors", topic:"aavss", title:"AAVSS — Sensors & Roles", text:"Roles:\n• LiDAR → 3D structure, obstacle shape, drivable free-space.\n• mmWave radar → range + radial velocity, robust in rain/fog, complements vision.\n• RGB camera → traffic lights/signs, lane markings, and VRU detection.\nExact SKUs vary by build; add your sensor models to docs.json or PDF for precise answers." },
    { id:"aavss-pipeline",topic:"aavss",title:"AAVSS — Fusion Pipeline (high level)",text:"Calibration → time-sync → per-sensor detection/feature extraction → object association → tracking → risk scoring/alerting. Safety analytics delivered in-cabin as HUD/alerts." },
    { id:"sld-overview", topic:"sldataset", title:"Sri Lanka Autonomous Driving Dataset — Overview", text:"Open driving dataset across Sri Lankan road scenarios (urban/rural; rain, fog, night). Includes annotations for lanes, signs, and hazards. Example media included; add PDFs for specs." },
    { id:"about-owner", topic:"about", title:"Ownership & Concept", text:`Owner/Concept/Manufacturer: ${OWNER.fullName}. Roles: ${OWNER.roles.join(", ")}. Primary projects: ${OWNER.projects.join(", ")}.` },
    { id:"about-whois", topic:"about", title:`Who is ${OWNER.fullName}?`, text:`${OWNER.fullName} — ${OWNER.tagline} Leads development and product direction for AAVSS and the Sri Lanka dataset efforts.` },
  ],
};
...
async function topK(question, k = 8, topic = "all") {
  ...
  const kbRanked = kbPool.map((d)=>({type:"kb",id:d.id,title:d.title,text:d.text,s:scoreText(qTok,`${d.title}\n${d.text}`)})).filter((x)=>x.s>0);
  const store = await readDocsStore();
  const pdfRanked = (store.docs||[]).map((ch)=>({type:"pdf",id:ch.id,title:ch.title,text:ch.text,page:ch.page,url:ch.url,s:scoreText(qTok,`${ch.title}\n${ch.text}`)})).filter((x)=>x.s>0);
...
async function askGroq({ system, user, signal }) {
  ...
  const r = await fetch("https://api.groq.com/openai/v1/chat/completions", {
    ...
    body: JSON.stringify({ model:"llama-3.1-70b-versatile", temperature:0.2, max_tokens:450, messages:[{role:"system",content:system},{role:"user",content:user}] })
  });
...
async function askDeepInfra({ system, user, signal }) {
  ...
  const r = await fetch("https://api.deepinfra.com/v1/openai/chat/completions", {
    method:"POST", signal,
    headers:{ Authorization:`Bearer ${key}`, "Content-Type":"application/json" },
    body: JSON.stringify({ model:"meta-llama/Meta-Llama-3.1-8B-Instruct", temperature:0.2, max_tokens:450, messages:[{role:"system",content:system},{role:"user",content:user}] })
  });
...
async function askGemini({ system, user, signal }) {
  ...
  const r = await fetch(url, {
    ...
    body: JSON.stringify({ contents:[{role:"user",parts:[{text:`${system}\n\n---\n\n${user}`}] }], generationConfig:{ temperature:0.2, maxOutputTokens:450 } })
  });
...
async function handler(req, res) {
  ...
  try {
    ...
    if (st) { res.writeHead(200, headers); res.end(JSON.stringify({ answer: st.reply, provider: "smalltalk", topic: "general", sources: [], followups: buildFollowups("all") })); return; }
    ...
    const msg = (err && err.name === "AbortError") ? "Upstream request timed out" : (err && err.message) || "Server error";
    res.writeHead(500, corsHeaders((req.headers && req.headers.origin) || "*"));
