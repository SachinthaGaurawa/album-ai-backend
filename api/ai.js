// Serverless endpoint: POST https://<your-backend>.vercel.app/api/ai
export const config = { runtime: 'edge' }; // fast & simple
...
export default async function handler(req, ctx) {
  ...
  try {
    ...
    if (mode === 'ask') {
      if (!question || !context) {
        return Response.json({ error: 'Missing question/context' }, { status: 400, headers });
      }
    ...
    if (mode === 'caption') {
      ...
      if (!capReq.ok) return Response.json(
        { error: `Vision HTTP ${capReq.status}: ${await capReq.text().catch(()=> '')}` },
        { status: 502, headers }
      );
      ...
      if (!tagReq.ok) return Response.json(
        { error: `Tags HTTP ${tagReq.status}: ${await tagReq.text().catch(()=> '')}` },
        { status: 502, headers }
      );
