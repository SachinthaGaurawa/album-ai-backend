...
/* --------------- Provider: Captions (vision via Gemini) --------------- */
...
async function captionWithGemini({ imageUrl, signal }) {
  ...
  const capReq = await fetch(endpoint, {
    ...
    body: JSON.stringify({
      contents: [
        {
          role: 'user',
          parts: [{ text: 'Describe this image in one concise, specific sentence.' }, { inline_data: { mime_type: mime, data: b64 } }],
        },
      ],
  ...
  const caption =
    (capJson?.candidates?.[0]?.content?.parts || [])
      .map((p) => p?.text || '')
      .join('')
      .trim() || '';
  ...
  const tagText =
    (tagJson?.candidates?.[0]?.content?.parts || [])
      .map((p) => p?.text || '')
      .join('')
      .trim() || '';
